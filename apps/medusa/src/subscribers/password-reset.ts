...
export default async function passwordResetHandler({ event, container }: SubscriberArgs<PasswordResetEvent>) {
  ...
  await createEmailAdapter().send({
    ...
    html: `<p>A password reset was requested for your Bangla Blend account.</p><p><a href="${safeUrl}">Choose a new password</a></p><p>This link can be used once and expires in 15 minutes. If you did not request it, you can ignore this email.</p>`,
