...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Please check your checkout details.", issues: parsed.error.issues },
      { status: 400 },
    );
  ...
  if (
    ...
    return NextResponse.json(
      { error: "Checkout is closed until the production release approvals are recorded." },
      { status: 503 },
    );
  const market = await getActiveMarket();
  if (!market.domestic && process.env.ENABLE_INTERNATIONAL_CHECKOUT !== "true")
    return NextResponse.json(
      { error: `Checkout is not yet enabled for ${market.label}.` },
      { status: 409 },
    );
  if (!market.domestic && parsed.data.paymentMethod !== "international")
    return NextResponse.json(
      { error: "Choose a configured international payment method." },
      { status: 400 },
    );
  if (market.domestic && parsed.data.paymentMethod === "international")
    return NextResponse.json(
      { error: "Choose Cash on Delivery or SSLCOMMERZ for Bangladesh." },
      { status: 400 },
    );
  if (parsed.data.shippingAddress.countryCode.toUpperCase() !== market.code.toUpperCase())
    return NextResponse.json(
      { error: `The delivery country must match the selected ${market.label} destination.` },
      { status: 409 },
    );
  if (parsed.data.paymentMethod === "cod" && process.env.MEDUSA_COD_ENABLED !== "true")
    return NextResponse.json(
      { error: "Cash on Delivery is not currently enabled." },
      { status: 409 },
    );
  if (
  ...
  if (!config.backendUrl || !config.publishableKey)
    return NextResponse.json({ error: "Commerce is not configured." }, { status: 503 });
  const headers = {
    "content-type": "application/json",
    "x-publishable-api-key": config.publishableKey,
  };
  const store = async <T>(path: string, init: RequestInit = {}) =>
    json<T>(
      await fetch(new URL(path, config.backendUrl), {
        ...init,
        cache: "no-store",
  ...
  try {
    let { cart } = await store<{ cart: CheckoutCart }>(
    ...
    if (!cart.shipping_methods?.length) {
      const options = await store<{ shipping_options: ShippingOption[] }>(
        `/store/shipping-options?cart_id=${encodeURIComponent(cartId)}&fields=%2Bcalculated_price`,
      );
      if (options.shipping_options.length > 1 && !parsed.data.shippingOptionId)
        return NextResponse.json(
          {
            error: "Choose a delivery method.",
            requiresShippingSelection: true,
            shippingOptions: options.shipping_options.map((option) => ({
              id: option.id,
              name: option.name ?? "Delivery",
              amount: option.calculated_price?.calculated_amount ?? option.amount,
              currencyCode:
                option.calculated_price?.currency_code?.toUpperCase() ?? market.currency,
            })),
          },
          { status: 409 },
        );
      const option = parsed.data.shippingOptionId
        ? options.shipping_options.find((item) => item.id === parsed.data.shippingOptionId)
        : options.shipping_options[0];
      if (!option)
        return NextResponse.json(
          { error: "No shipping method is configured for this address." },
          { status: 409 },
        );
      ({ cart } = await store<{ cart: CheckoutCart }>(
    ...
    if (!paymentCollectionId) {
      const collection = await store<{ payment_collection: { id: string } }>(
    ...
    const providerId =
      parsed.data.paymentMethod === "cod"
        ? (process.env.MEDUSA_COD_PROVIDER_ID ?? "pp_system_default")
        : parsed.data.paymentMethod === "sslcommerz"
          ? (process.env.MEDUSA_SSLCOMMERZ_PROVIDER_ID ?? "pp_sslcommerz_sslcommerz")
          : process.env.MEDUSA_INTERNATIONAL_PROVIDER_ID;
    if (!providerId)
      return NextResponse.json(
        { error: "No payment provider is configured for this market." },
        { status: 409 },
      );
    const session = await store<{
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          customer_country: billingAddress.country_code,
        },
      }),
    });
    const activeSession = session.payment_collection.payment_sessions?.find(
      (item) => item.provider_id === providerId,
    );
    const gatewayUrl = activeSession?.data?.gateway_url;
    if (parsed.data.paymentMethod === "sslcommerz" && !validGatewayUrl(gatewayUrl))
      return NextResponse.json(
        { error: "The payment gateway did not return a secure redirect URL." },
        { status: 502 },
      );
    ...
    }>(`/store/carts/${encodeURIComponent(cartId)}/complete`, { method: "POST" });
    if (completion.type !== "order" || !completion.order)
      return NextResponse.json(
        { error: completion.error?.message ?? "The cart could not be completed." },
        { status: 409 },
      );
    ...
    const result = NextResponse.json({
      redirect: `/checkout/success?order=${encodeURIComponent(completion.order.id)}`,
      orderId: completion.order.id,
    ...
    return NextResponse.json(
      { error: `Checkout could not be completed.${detail}` },
      { status: 503 },
    );
  }
}
