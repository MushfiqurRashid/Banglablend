...
export async function GET(request: Request) {
  ...
  if (query.length > 120) return NextResponse.json({ error: "Search is too long." }, { status: 400 });
  ...
  if (process.env.NODE_ENV === "development" && process.env.ENABLE_DEVELOPMENT_FALLBACKS === "true") {
    ...
    return NextResponse.json({ hits: products.map((product) => ({ id: product.id, type: product.collection === "gifts" ? "gift" : "product", title: product.title, slug: product.handle, excerpt: product.description, eligibleMarkets: product.eligibleMarkets })), estimatedTotalHits: products.length, source: "development-preview" });
  }
