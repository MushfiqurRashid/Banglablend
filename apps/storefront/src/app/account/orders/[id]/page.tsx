...
export default async function AccountOrderPage({
  ...
  const payload = await customerStoreFetch<{ order: OrderDetail }>(
    `/store/orders/${encodeURIComponent(id)}?fields=*items`,
  );
  ...
  return (
    <>
      ...
      <div className="account-panel" style={{ marginTop: "2rem" }}>
        ...
        {order.metadata?.is_gift ? (
          <p>
            This order contains instructions for the gift recipient. Support can help with approved
            changes.
          </p>
        ) : null}
      </div>
